...
const sleep = function(time) {
    time = time || (Math.floor(Math.random() * 10 + 5)) * 1000
...
const httpRequest = async(options) => {
    ...
    //获取城市列表
const fetchPage = async(total, rn) => {
    console.log(`开始抓取城市基础数据：${new Date().toLocaleDateString()}  ${new Date().toLocaleTimeString()}`)
    rn = rn || 100
    let pnNum = Math.ceil(total / rn)
    let result = []
    for (let pn = 1; pn <= pnNum; pn++) {
        let options = {
            uri: `${hostName}destination/ajax/jingdian?format=ajax&cid=0&playid=0&seasonid=5&surl=zhongguo&pn=${pn}&rn=${rn}`,
            headers: {
        ...
        let finalDataResult = await Citys.create(finalData)
        result = result.concat(finalDataResult)
        console.log(`第${pn}页所有城市基本数据保存成功`)
    }
    console.log(`所有城市基本数据保存成功`)
    await getCityExtraMessage(result, request_id)
}

//获取对应城市额外数据
const getCityExtraMessage = async(result, request_id, cityIndex = 0, attractionIndex = 1) => {
    for (let j = cityIndex; j <= result.length - 1; j--) {
        let data = result[j]
        let options = {
        ...
        while (attractionIndex <= AttractionsPages) {
            await sleep()
            await getAttractionsCity(attractionIndex, updateCityData)
            attractionIndex++
        }
